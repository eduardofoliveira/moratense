...
const connectionManagerHostgator_1 = __importDefault(require("./database/connectionManagerHostgator"));
...
const executar = async () => {
    ...
    const connHG = connectionManagerHostgator_1.default.getConnection();
    const stream = connHG
        .select("*")
...
const fixColaboradoresLegado = async () => {
    try {
        const connTeleconsult = connectionManager_1.default.getConnection();
        const connMoratense = connectionManagerHomeLab_1.default.getConnection();
        ...
        ORDER BY
          qtd desc
    `);
        funcionariosDuplicados = funcionariosDuplicados.filter((item) => item.qtd > 1);
        for await (const funcionario of funcionariosDuplicados) {
            const [funcionarios] = await connTeleconsult.raw(`SELECT * FROM colaboradores WHERE codigo = ${funcionario.codigo} and id_empresa = 4`);
